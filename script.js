...
const validKeys = ["key1", "key2", "key3"]; // Add your keys here

// Function to execute the script if the key is valid
function executeScript() {
    init();
}

// Prompt the user to enter a key
const userInput = prompt("Enter your access key:");

// Check if the entered key is valid
if (validKeys.includes(userInput)) {
    // If the key is valid, execute the script
    executeScript();
} else {
    // If the key is invalid, display an error message
    alert("Invalid access key. Please purchase a valid key to access the script.");
}
